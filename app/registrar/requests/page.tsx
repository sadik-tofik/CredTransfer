...
function StatusBadge({ status }: { status: string }) {
  const cfg: Record<string, { cls: string; label: string }> = {
    pending: {
      cls: "bg-amber-900/50 text-amber-300 border-amber-700/50",
      label: "Pending",
    },
    approved: {
    ...
    rejected: {
      cls: "bg-red-900/50 text-red-300 border-red-700/50",
      label: "Rejected",
    },
    completed: {
...
export default function RegistrarRequestsPage() {
  ...
  const [selectedRequest, setSelectedRequest] =
    useState<ExtendedTransferRequest | null>(null);
  ...
  const fetchRequests = useCallback(async () => {
    try {
      const response = await axios.get(
        `/api/transfers/pending?status=${filter === "all" ? "" : filter}`,
      );
  ...
  const loadScreenshot = useCallback(
    async (paymentId: string, screenshotPath: string) => {
      setLoadingScreenshot(true);
      try {
        const response = await axios.get(
          `/api/payments/screenshot-url?payment_id=${paymentId}`,
        );
  ...
  const handleAction = async () => {
    ...
    try {
      ...
      toast.success(
        `Transfer ${actionType === "approve" ? "approved" : "rejected"} successfully!`,
      );
      ...
      toast.error(
        error?.response?.data?.error || `Failed to ${actionType} request`,
      );
    } finally {
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {screenshotPendingCount > 0 && (
            <Badge className="bg-blue-900/50 text-blue-300 border-blue-700/50 text-xs">
              🔍 {screenshotPendingCount} screenshot
              {screenshotPendingCount > 1 ? "s" : ""} to verify
            </Badge>
          )}
          {pendingCount > 0 && (
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          <p className="text-white font-semibold text-sm">
                            {req.graduate?.user?.full_name ||
                              "Unknown Graduate"}
                          </p>
                          <span className="text-white/30 text-xs">
                            {req.graduate?.student_id}
                          </span>
                          {needsReview && (
                        ...
                        <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                          <span className="text-white/40 text-xs">
                            {req.document
                              ? getDocumentTypeLabel(req.document.document_type)
                              : "Document"}
                          </span>
            ...
            <div className="space-y-4 mt-2">
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                <div className="p-4 bg-white/5 rounded-xl space-y-2.5">
                  ...
                  <InfoRow
                    label="Name"
                    value={selectedRequest.graduate?.user?.full_name || "—"}
                  />
                  <InfoRow
                    label="Student ID"
                    value={selectedRequest.graduate?.student_id || "—"}
                    mono
                  />
                  {selectedRequest.graduate?.department && (
                    <InfoRow
                      label="Department"
                      value={selectedRequest.graduate.department}
                    />
                  )}
                </div>
                <div className="p-4 bg-white/5 rounded-xl space-y-2.5">
                    ...
                    value={
                      selectedRequest.document
                        ? getDocumentTypeLabel(
                            selectedRequest.document.document_type,
                          )
                        : "—"
                    }
                  />
                  <InfoRow
                    label="Recipient"
                    value={selectedRequest.recipient_institution}
                  />
                  {(selectedRequest as ExtendedTransferRequest)
                    .university_email && (
                    <InfoRow
                      label="Univ. Email"
                      value={
                  ...
                  <InfoRow
                    label="Expires"
                    value={formatDate(selectedRequest.expires_at)}
                  />
                </div>
              </div>
              ...
              {selectedRequest.payment && (
                <div className="p-4 bg-white/5 rounded-xl space-y-2.5">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <InfoRow
                      label="Amount"
                      value={`${selectedRequest.payment.amount} ETB`}
                    />
                    <InfoRow
                      label="Method"
                    ...
                    {selectedRequest.payment.transaction_reference && (
                      <InfoRow
                        label="Reference"
                        value={selectedRequest.payment.transaction_reference}
                        mono
                      />
                    )}
              ...
              {selectedRequest.payment?.payment_screenshot_url && (
                <div className="space-y-2">
                  ...
                  <p className="text-white/30 text-xs">
                    Verify that the payment amount is 500 ETB and the
                    transaction appears successful before approving.
                  </p>
                </div>
              )}
                  ...
                  <div className="flex items-center gap-3 p-4 bg-amber-500/10 border border-amber-500/25 rounded-xl">
                    ...
                    <div>
                      ...
                      <p className="text-amber-200/60 text-xs mt-0.5">
                        The graduate has not yet uploaded a payment screenshot.
                        You may still approve if you have confirmed payment
                        through other means.
                      </p>
                    </div>
                  </div>
...
function InfoRow({
  ...
  return (
    <div className="flex items-center justify-between gap-4">
      ...
      <span
        className={`text-white text-sm text-right ${mono ? "font-mono" : "font-medium"}`}
      >
        {value}
      </span>
    </div>
...
function ScreenshotFallback({
  ...
  useEffect(() => {
    axios
      .get(`/api/payments/screenshot-url?payment_id=${paymentId}`)
    ...
    return (
      <div className="h-48 flex flex-col items-center justify-center gap-2 text-white/30">
        ...
        <p className="text-xs text-white/20">
          Configure Supabase Storage to view inline.
        </p>
      </div>
    );
  }
