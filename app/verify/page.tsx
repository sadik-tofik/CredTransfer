...
export default function VerifyPage() {
  ...
  const [code, setCode] = useState(
    searchParams.get("code") || searchParams.get("hash") || "",
  );
  ...
  const handleVerify = async (verifyCode?: string) => {
    ...
    try {
      const response = await axios.get(
        `/api/verifications/public/${encodeURIComponent(codeToVerify.trim())}`,
        {
      ...
      <div className="max-w-4xl mx-auto px-4 py-12">
        <div className="text-center mb-10">
          ...
          <p className="text-white/60">
            Verify the authenticity of academic credentials from Jimma
            University
          </p>
        </div>
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <Label className="text-white/80">
                Verification Code or Document Hash
              </Label>
            ...
            <div className="space-y-2">
              <Label className="text-white/60 text-sm">
                Your Institution (Optional - for audit purposes)
              </Label>
              <Input
                value={institution}
            ...
            <div className="flex items-center gap-4 pt-2">
              <input
                ...
                onChange={(e) => {
                  ...
                  if (file)
                    toast.info(
                      "QR scanning coming soon. Please enter the code manually.",
                    );
                }}
              />
              <Button
          ...
          <Card className={`border-2 ${config.bg} mb-6`}>
            <CardContent className="p-8">
              ...
              <div className="text-center mb-8">
                ...
                <div
                  className={`inline-block px-8 py-3 rounded-full text-2xl font-bold ${config.badge} shadow-lg`}
                >
                  {config.text}
                </div>
                ...
                <div className="grid md:grid-cols-2 gap-8">
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      <InfoRow
                        label="Full Name"
                        value={result.graduate.full_name}
                      />
                      <InfoRow
                        label="Student ID"
                        value={result.graduate.student_id}
                        mono
                      />
                      {result.graduate.department && (
                        <InfoRow
                          label="Department"
                          value={result.graduate.department}
                        />
                      )}
                      {result.graduate.graduation_year && (
                        <InfoRow
                          label="Graduation Year"
                          value={result.graduate.graduation_year.toString()}
                        />
                      )}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      ...
                      <InfoRow
                        label="File Name"
                        value={result.document.file_name}
                      />
                      <InfoRow
                        label="Upload Date"
                        value={formatDate(result.document.uploaded_at)}
                      />
                      <InfoRow
                        label="SHA-256 Hash"
                        value={truncateHash(result.document.file_hash, 8, 6)}
                        mono
                      />
                    </div>
                  </div>
                ...
                <>
                  ...
                  <div>
                    ...
                    <div className="bg-black/30 rounded-lg p-4 space-y-3">
                      <InfoRow
                        label="Network"
                        value={result.blockchain.network || "zkSync Sepolia"}
                        badge
                      />
                      ...
                      {result.blockchain.block_number && (
                        <InfoRow
                          label="Block Number"
                          value={result.blockchain.block_number.toString()}
                          mono
                        />
                      )}
                      {result.blockchain.timestamp && (
                        <InfoRow
                          ...
                          value={formatDateTime(
                            new Date(
                              result.blockchain.timestamp * 1000,
                            ).toISOString(),
                          )}
                        />
                      )}
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      <p className="text-white/60 text-sm">
                        This document has been securely shared with your
                        institution. You can download a verified copy below.
                      </p>
                      <Button
                        onClick={() => {
                          ...
                          link.download =
                            (result as any).document?.file_name ||
                            "document.pdf";
                    ...
                    <div className="space-y-3">
                      <InfoRow
                        ...
                        value={
                          (
                            result as {
                              transfer?: { recipient_institution: string };
                            }
                      ...
                      <InfoRow
                        label="Valid Until"
                        value={formatDate(
                          (result as { transfer?: { expires_at: string } })
                            .transfer?.expires_at || "",
                        )}
                      />
                    </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-white/50 text-sm">
                      Academic Registrar&apos;s Office, Ethiopia
                    </p>
                  </div>
                </div>
            ...
            {[
              {
                icon: "🔑",
                title: "Hash Code",
                desc: "Enter the 16-character alphanumeric code from your document",
              },
              {
                icon: "📷",
                title: "QR Code",
                desc: "Upload or scan the QR code attached to the document",
              },
              {
                icon: "🔗",
                title: "Document Hash",
                desc: "Enter the full SHA-256 hash of the document file",
              },
            ].map((item) => (
              <Card
                key={item.title}
  ...
  return (
    <div className="flex items-center justify-between gap-4">
      ...
      {badge ? (
        ...
        <span
          className={`text-white text-sm text-right ${mono ? "font-mono" : ""}`}
        >
          {value}
        </span>
      )}
