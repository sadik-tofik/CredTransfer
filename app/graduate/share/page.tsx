...
const PAYMENT_ACCOUNTS = {
  cbe_birr: {
    ...
    icon: "🏦",
    instructions:
      "Send exactly 500 ETB to the CBE Birr account below, then take a screenshot.",
    fields: [
      {
        label: "Account Name",
        value: "Jimma University – CredTransfer",
        copyable: false,
      },
  ...
  telebirr: {
    ...
    icon: "📱",
    instructions:
      "Send exactly 500 ETB to the TeleBirr number below, then take a screenshot.",
    fields: [
      {
        label: "Account Name",
        value: "Jimma University – CredTransfer",
  ...
  bank_transfer: {
    ...
    icon: "🏛️",
    instructions:
      "Wire 500 ETB to the account below. Include the reference number in the description.",
    fields: [
      ...
      {steps.map((label, i) => (
        <div key={label} className="flex items-center">
          <div className="flex flex-col items-center gap-1.5">
            ...
            <span
              className={`text-xs font-medium hidden sm:block ${i === current ? "text-white" : "text-white/30"}`}
            >
              {label}
            </span>
          </div>
          {i < steps.length - 1 && (
            <div
              className={`w-12 sm:w-20 h-px mx-2 mb-5 transition-all ${i < current ? "bg-emerald-500" : "bg-white/10"}`}
            />
          )}
        </div>
      ))}
    ...
    return (
      <div className="flex flex-col items-center gap-3 py-8 text-center">
        ...
        <p className="text-white/50 text-sm max-w-sm">
          The registrar will review your payment proof and approve the transfer.
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-white/40 text-xs">
          Take a clear screenshot of the successful transaction confirmation and
          upload it below.
        </p>
      </div>
      ...
      >
        ...
        {preview ? (
          <div className="p-3">
            ...
            />
            <p className="text-center text-emerald-400 text-xs mt-2 font-medium">
      ...
      <div className="p-6 max-w-2xl mx-auto space-y-6">
        <div>
          ...
          <p className="text-white/50 text-sm mt-1">
            Share your academic credentials with a receiving institution
          </p>
        </div>
          ...
          <div className="flex items-start gap-3 p-4 bg-amber-500/10 border border-amber-500/25 rounded-xl">
            ...
            <div>
              <p className="text-amber-300 font-semibold text-sm">
                Service Fee Required: {SERVICE_FEE} ETB
              </p>
              <p className="text-amber-200/60 text-xs mt-0.5">
                A one-time fee is needed to process the transfer. Payment
                instructions appear in the next step.
              </p>
            </div>
          </div>
        )}
          ...
          <CardContent className="p-6">
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <Select
                  value={selectedDocId ?? ""}
                  onValueChange={(v) => setValue("document_id", v || "")}
                >
                ...
                {selectedDocument && (
                  <div className="flex items-center gap-3 p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-lg mt-2">
                    ...
                    {selectedDocument.blockchain_tx_hash && (
                      <Badge className="bg-purple-900/50 text-purple-300 border-purple-700/50 text-xs flex-shrink-0">
                        ⛓️ On-chain
                      </Badge>
                    )}
                  </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  {errors.recipient_institution && (
                    <p className="text-red-400 text-xs">
                      {errors.recipient_institution.message}
                    </p>
                  )}
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <p className="text-white/30 text-xs">
                    The receiving university will get an email with the
                    verification QR code and hash once approved.
                  </p>
                </div>
                ...
                <>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      {(
                        Object.keys(PAYMENT_ACCOUNTS) as Array<
                          keyof typeof PAYMENT_ACCOUNTS
                        >
                      ).map((method) => {
      ...
      <div className="p-6 max-w-2xl mx-auto space-y-6">
          ...
          <CardContent className="space-y-0">
            ...
            {paymentMethod === "bank_transfer" && (
              <p className="text-amber-300/60 text-xs pt-3">
                ⚠️ You must include the reference number in your bank transfer
                description for the registrar to match your payment.
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="bg-white/5 border-white/10">
          <CardHeader className="pb-3">
            ...
            <CardDescription className="text-white/50 text-sm">
              After completing the payment, upload a screenshot of the
              confirmation screen.
            </CardDescription>
          </CardHeader>
          <CardContent>
          ...
          <CardContent className="p-8 text-center space-y-6">
            ...
            <div className="text-left space-y-3 p-4 bg-white/5 rounded-xl">
              ...
              {[
                ...
                {
                  text: "Registrar verifies payment and approves request",
                  done: false,
                },
                {
                  text: "Receiving university receives email with QR code and verification hash",
                  done: false,
                },
                {
                  text: "You can view your verification codes in Transfer History",
                  done: false,
                },
              ].map((item, i) => (
                <div key={i} className="flex items-start gap-3">
                  <div
                    className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 mt-0.5 ${item.done ? "bg-emerald-500 text-white" : "bg-white/10 text-white/30"}`}
                  >
                    {item.done ? "✓" : i + 1}
                  </div>
                  <p
                    className={`text-sm ${item.done ? "text-white/80" : "text-white/50"}`}
                  >
                    {item.text}
                  </p>
                </div>
            ...
            {feeCleared && shareResult.hash_code && (
              <div className="p-4 bg-emerald-500/10 border border-emerald-500/25 rounded-xl">
                ...
                <p className="text-emerald-300 font-mono font-bold text-2xl tracking-widest">
